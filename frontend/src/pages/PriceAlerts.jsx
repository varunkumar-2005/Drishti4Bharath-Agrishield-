...
export default function PriceAlerts() {
  ...
  // Extract price impacts from events
  ...
  return (
    <div className="p-7 animate-fadeIn">
      ...
      <div className="grid grid-cols-4 gap-4 mb-6">
        <KpiCard label="Active Alerts" value={upAlerts.length} color="red"
          sub="Price rise predictions" badge="URGENT" />
        <KpiCard label="Price Falls" value={downAlerts.length} color="green"
          sub="Opportunities for consumers" />
        <KpiCard label="Max Price Impact" value={`${maxCPI.toFixed(1)}%`} color="yellow"
          sub="Largest single commodity swing" />
        <KpiCard label="Events Tracked" value={events?.length || 0} color="blue"
          sub="Active geopolitical events" />
      </div>
            ...
            allPriceImpacts.map((p, i) => {
              ...
              return (
                <div key={i} className="flex items-center gap-3.5 px-5 py-3.5 border-b cursor-pointer transition-colors hover:bg-white/[.015]"
                  ...
                  <div className="w-11 h-11 rounded-full flex items-center justify-center font-syne font-extrabold text-[12px] flex-shrink-0"
                    style={circleStyle}>
                  ...
                  <div className="text-right flex-shrink-0">
                    <div className="text-[12px] font-bold"
                      style={{ color: isUp ? 'var(--danger)' : 'var(--accent)' }}>
